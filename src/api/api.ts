import axios, {AxiosResponse} from "axios";
import {StrapiResponseForArticle} from "./types";

const token = "";

const instance = axios.create({
    baseURL: "http://192.168.0.28:1337/api",
    timeout: 1000,
    headers: {"Authorization": `Bearer ${token}`}
});



export const getArticles = (): Promise<AxiosResponse<StrapiResponseForArticle>>  => {
    return instance.get("articles");
};


// export const getAuthor = (id: string): Promise<AxiosResponse<StrapiResponseForArticle>>  => {
//     return instance.get(`author/${id}`);
// };
